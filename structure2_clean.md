...
> GOLDEN RULE Every section listed here must appear in every Wikipedia, in the exact order shown. If a section has no data yet, the agent writes a "📭 No data found" placeholder and specifies what type of source is needed. Sections are never skipped, merged, or reordered.
...
| 3      | Listing Spec Tiers               | Primary / Secondary / Tertiary classification | Tier tables                |
| 4      | Buyer Specifications             | What buyers ask + miss                        | Priority matrix + gap list |
| 5      | Most Relevant Spec Combinations  | Reference configurations                      | Config tier table          |
| 6      | Spec Contradictions & Flags      | Conflicts, violations, red flags              | Warning blocks             |
| 7      | Price-Defining Specs & Variation | Price intelligence                            | Price stack + map          |
| 8      | Buyer Personas                   | Who buys + how they behave                    | Persona profiles           |
| 9      | Seller Personas                  | Who sells + supplier risk                     | Persona profiles           |
...
> STRUCTURAL RULE: 15 FIXED SUBSECTIONS Section 2 always has exactly 15 subsections (2.1 through 2.15), in the same order, with the same names. The agent adapts the content inside each subsection to the specific category. Subsections marked "always present" must never be empty — if no data exists, explicitly state what data is needed. Subsections not applicable to a category get a one-line note explaining why (e.g., "2.10 Operator Interface: Not applicable for raw material bulk commodities — no operator interface exists.") Never delete a subsection.
...
> 3 Listing Spec Tiers Classifying every spec from Section 2 into Primary, Secondary, and Tertiary tiers
...
> 4 Buyer Specifications What buyers actually ask, what they prioritise, and what they miss
...
> 5 Most Relevant Spec Combinations Reference configurations from entry to premium — the spec decision tree made into tables
...
> 6 Spec Contradictions &amp; Flags Hard conflicts, regulatory violations, terminology ambiguity, and buyer red-flag combinations
...
> 7 Price-Defining Specs &amp; Price Variation What moves the price needle, by how much, and why prices vary across India
...
> 8 Buyer Personas Detailed profiles of every distinct buyer type in this category
...
> 9 Seller Personas Detailed profiles of every distinct seller type and their risk profile
...
| Quick Facts               | High / Medium / Low  | \[Reason based on source coverage\]            |
| 1\. Category Overview     | High / Medium / Low  | \[Reason\]                                     |
| 2\. Seller Specifications | High / Medium / Low  | \[Reason — note which subsections are thin\]   |
| 3\. Listing Spec Tiers    | High / Medium / Low  | \[Reason\]                                     |
| 4\. Buyer Specifications  | High / Medium / Low  | \[Reason — note if buyer call data is sparse\] |
| 5\. Spec Combinations     | High / Medium / Low  | \[Reason\]                                     |
| 6\. Contradictions        | High / Medium / Low  | \[Reason\]                                     |
| 7\. Price Intelligence    | High / Medium / Low  | \[Reason — price data is often sparse\]        |
| 8\. Buyer Personas        | High / Medium / Low  | \[Reason\]                                     |
| 9\. Seller Personas       | High / Medium / Low  | \[Reason\]                                     |
...
A running list of unresolved questions flagged during Wikipedia generation. These are reviewed by a human editor before the Wikipedia is marked as "verified". Format each doubt as:

|              |                |                                                                                     |                                                     |                     |                                                     |
|--------------|----------------|-------------------------------------------------------------------------------------|-----------------------------------------------------|---------------------|-----------------------------------------------------|
| **Doubt \#** | **Section**    | **Question**                                                                        | **Evidence**                                        | **Severity**        | **Action Taken**                                    |
| 001          | \[Section \#\] | \[The specific question — e.g., "Does 42-inch refer to seat width or body width?"\] | Source A: \[value\]. Source B: \[different value\]. | High / Medium / Low | \[What was written in the wiki pending resolution\] |
| 002          | \[Section \#\] | \[Question\]                                                                        | \[Evidence\]                                        | \[Severity\]        | \[Action\]                                          |

**Quick Reference Summary**

The table below is the complete one-page reference for the universal Wikipedia structure. Print this or pin it — it is the master checklist every agent run should follow.

|             |                       |                     |                                       |                                |                                                   |
|-------------|-----------------------|---------------------|---------------------------------------|--------------------------------|---------------------------------------------------|
| **Section** | **Name**              | **Always Present?** | **Min Subsections**                   | **Primary Data Source**        | **Output If No Data**                             |
| QF          | Quick Facts           | YES                 | 8 fields                              | All sections combined          | Leave blank — populate last                       |
| 1           | Category Overview     | YES                 | 6 subsections                         | Web + call patterns            | "📭 No data found — needs \[type\]"               |
| 2           | Seller Specifications | YES                 | 15 subsections (2.1–2.15)             | Seller PDFs + catalogs         | "📭 No data — subsection needs \[source type\]"   |
| 3           | Listing Spec Tiers    | YES                 | 3 tier tables (P/S/T)                 | Derived from Sections 2 + 4    | "📭 Insufficient data — needs call analysis"      |
| 4           | Buyer Specifications  | YES                 | 5 subsections                         | Buyer call transcripts         | "📭 No buyer call data ingested"                  |
| 5           | Spec Combinations     | YES                 | 4 subsections                         | Calls + PDFs combined          | "📭 Insufficient data"                            |
| 6           | Spec Contradictions   | YES                 | 5 subsections                         | Cross-source comparison        | "📭 No contradictions found — may be sparse data" |
| 7           | Price Intelligence    | YES                 | 6 subsections                         | Calls + web pricing            | "📭 No pricing data in current sources"           |
| 8           | Buyer Personas        | YES                 | 1 profile per buyer type              | Buyer call transcripts         | "📭 Fewer than 2 buyer types identified"          |
| 9           | Seller Personas       | YES                 | 1 profile per seller type             | Call + catalog patterns        | "📭 Fewer than 2 seller types identified"         |
| G           | Glossary              | YES                 | 4 buckets                             | All sections — extracted terms | "📭 No specialist terms found"                    |
| M           | Wiki Metadata         | YES                 | 3 tables (fields, confidence, doubts) | Agent-generated                | Always populated — never empty                    |

*End of Universal B2B Wikipedia Structure Guide*
